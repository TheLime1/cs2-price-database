...
class CSGODatabaseFallback:
        ...
        try:
                    ...
                    try:
                        ...
                        if cells:
                            ...
                            if 'steam' in first_cell_text or i == 1:  # Steam is usually first data row
                                steam_row = row
                                logger.info(
                                    f"🎯 Found Steam row: {first_cell_text}")
                                break
                    except Exception as e:
            ...
            try:
                ...
                for i, cell in enumerate(steam_cells[1:], 1):
                    if i < len(headers):
                        wear_condition = self._extract_wear_condition_from_header(
                            headers[i])
                        if wear_condition:
                            ...
                            if price:
                                logger.info(
                                    f"  💰 {wear_condition}: ${price:.2f}")
                            else:
                                logger.info(
                                    f"  ❌ {wear_condition}: No listing ({price_text})")

                logger.info(
                    f"✅ Fallback: Scraped {len(prices)} wear conditions for {skin_name}")
                return prices
            ...
            logger.error(
                f"❌ Fallback timeout waiting for page load: {skin_name}")
            return {}
    ...
    async def get_fallback_prices(self, detail_url: str, skin_name: str, required_wears: List[str]) -> Dict[str, Tuple[Optional[float], Optional[float]]]:
        ...
        for wear in required_wears:
            ...
            # StatTrak not available from scraping
            result[wear] = (normal_price, None)
