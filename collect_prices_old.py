...
class V2PriceCollector:
    ...
    def load_checkpoint(self) -> Dict:
        ...
        if os.path.exists(self.checkpoint_path):
            try:
                ...
                logger.info(f"Loaded checkpoint: processed {checkpoint.get('processed_skins', 0)} skins")
                return checkpoint
            except Exception as e:
    ...
    def filter_missing_items(self, data: Dict, limit: Optional[int] = None) -> List[SkinItem]:
        ...
        if limit:
            logger.info(f"Limited to first {limit} skins ({total_variants} total variants)")
            
        logger.info(f"Processing {total_variants} missing price entries from {len(missing_items)} skins")
    ...
    async def collect_prices(self, missing_only: bool = False, limit: Optional[int] = None, resume: bool = True):
        ...
        if missing_only:
            logger.info("Missing-only mode: Building queue of items needing price updates")
            
            # Filter and load only missing items
            missing_items = self.filter_missing_items(data, limit)
            
            if not missing_items:
                logger.info("No missing prices found - database is up to date!")
                return
    ...
    async def shutdown(self):
        ...
        logger.info(f"CS2 PRICE COLLECTION V2.0 COMPLETED in {duration.total_seconds():.2f} seconds!")
        ...
        logger.info("Collection completed. Summary report saved to logs/summary.txt")
...
class PriceCollector:
    ...
    def __init__(self, ignore_stattrak: bool = False, missing_only: bool = False, debug: bool = False, noproxy: bool = False, no_fallback: bool = False, fallback_only: bool = False, update_availability: bool = False):
        ...
        # Proxy configuration
        if self.noproxy:
            self.proxy_manager.use_proxies = False
    ...
    async def process_variants_concurrently(self, skin: Dict, variants: List[Dict], skin_index: int, total_skins: int) -> None:
        ...
        if hasattr(self.proxy_manager, 'max_concurrent_requests'):
            max_concurrent = self.proxy_manager.max_concurrent_requests
    ...
    async def collect_all_prices(self, limit: Optional[int] = None, resume: bool = True):
        """V2.0 HIGH-SPEED COLLECTION - The only collection method"""
        logger.info("� Starting CS2 Price Collection System V2.0")
        ...
        # For missing-only mode, build task queue
        if self.missing_only:
            logger.info("🎯 Missing-only mode: Building queue of items needing price updates")
            missing_tasks = self.build_missing_items_queue()

            if not missing_tasks:
                logger.info("✅ All items already have prices - nothing to process!")
                return

            # Apply skin-based limit correctly
            if limit is not None and limit > 0:
                # Group tasks by skin_id to count actual skins
                skin_groups = {}
                for skin, variant, is_stattrak in missing_tasks:
                    skin_id = skin['id']
                    if skin_id not in skin_groups:
                        skin_groups[skin_id] = []
                    skin_groups[skin_id].append((skin, variant, is_stattrak))
                
                # Take only the first N skins
                selected_skins = list(skin_groups.keys())[:limit]
                missing_tasks = []
                for skin_id in selected_skins:
                    missing_tasks.extend(skin_groups[skin_id])
                
                logger.info(f"🎯 Limited to first {limit} skins ({len(missing_tasks)} total variants)")

            self.stats['total_variants'] = len(missing_tasks)
            self.stats['total_skins'] = len(set(task[0]['id'] for task in missing_tasks))
            self.stats['start_time'] = datetime.now()

            logger.info(f"📊 Processing {len(missing_tasks)} missing price entries from {self.stats['total_skins']} skins")

            await self._process_missing_items_queue(missing_tasks)
            
        else:
            # Sequential mode: process skins in order
            skins_with_dates = self.sort_skins_by_date()
            self.stats['total_skins'], self.stats['total_variants'] = self.calculate_total_work(skins_with_dates)
            self.stats['start_time'] = datetime.now()

            logger.info(f"Total work: {self.stats['total_skins']} skins, {self.stats['total_variants']} price requests")

            # Apply skin-based limit correctly
            if limit is not None and limit > 0:
                skins_with_dates = skins_with_dates[:limit]
                logger.info(f"🎯 Limited to first {limit} skins for processing")
            elif limit == 0:
        ...
        logger.info(f"🎉 CS2 PRICE COLLECTION V2.0 COMPLETED in {total_time:.2f} seconds!")
...
async def main():
    ...
    # V2.0 startup message
    logger.info("🚀" + "="*80)
    logger.info("🚀 CS2 PRICE COLLECTION SYSTEM V2.0")
    logger.info("🚀 High-performance price collection with enhanced features")
    logger.info("🚀" + "="*80)
